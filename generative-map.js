...
function drawQuadrant(voffset, hoffset, hbound, vbound) {
  ...
  for (let anchorIdx = 1; anchorIdx < anchorNum; anchorIdx++) {
    let randomX = random(
      hmid + hincrem * (anchorIdx - 1),
      hmid + hincrem * anchorIdx
    );
  ...
  for (let anchorIdx = 1; anchorIdx < curLen; anchorIdx++) {
    let v = createVector(
      anchors[curLen - anchorIdx].x - random(-15, 15),
      anchors[curLen - anchorIdx].y + vincrem * anchorIdx
    );
    anchors.push(v);
  }
